import 'package:ocr_parser/src/core_types/predicates/datetime/is_valid_date_predicate.dart';
...
import '../../core_types/predicates/and_predicate.dart';
...
class GetDeiBoundingBoxMapUsecase implements Usecase<BoundingBoxMap, Point<double>?> {
  @override
  Future<BoundingBoxMap> execute(Point<double>? offset) {
    final map = {
      'rfCode': TargetWord(
    ...
    if (offset != null) {
      final correctedMap = map.map(
        (key, value) =>
            MapEntry(key, value.mapBoundingBox((boundingBox) => boundingBox.translate(offset))),
      );
      return Future.value(correctedMap);
    } else {
